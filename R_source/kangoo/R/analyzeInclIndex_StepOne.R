...
analyzeInclIndex <- function(plate.setup="splicing",plate.format="96-well"){
  ...
  if(plate.setup == "splicing"){
    sample.pattern <- c("Cells","Treatment","Gene","Primer")
  }
  ...
  raw.df <- read.delim(file=exp.link,skip=8,nrows=nwells,stringsAsFactors=F)
  # removing trailing information
  raw.df <- raw.df[c(1:(grep(pattern="Analysis Type",raw.df$Well)-1)),]
  # remove empty wells
  # raw.df <- raw.df[grep(pattern=".*_.*",raw.df$Sample.Name),]
  # split sample name column
  df <- data.frame(raw.df$Well
                   ,colsplit(raw.df$Sample.Name," ",c("Treatment","Time"))
                   ,colsplit(raw.df$Target.Name,"_",c("Gene","Primer"))
                   ,raw.df$Reporter
                   ,as.numeric(raw.df[,10])
                   ,raw.df$Ct.Threshold  
  )
  ...
  df.m <- melt(data=df
               ,id.vars=c("raw.df.Well","Gene","Primer","Treatment")
               ,measure.vars="Ct"
  )
  ...
  res.ca <- dcast(data = df.m
                  ,formula = raw.df.Well + Gene + Primer ~ Treatment
                  ,value.var = "value"
  )
  ...
  res.m <- melt(res.ca,id.vars=c("raw.df.Well","Gene","Primer"),measure.vars=treatment)
